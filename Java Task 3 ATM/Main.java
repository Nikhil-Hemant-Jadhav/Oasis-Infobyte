...
class Account {
    ...
    void createAcc() {
        ...
        // To Create a file with the account number
        ...
        try {
            ...
            writer.close();
            //System.out.println("File "+ fileNae + " created successfully.");
        } catch (IOException e) {
...
class ATM {
        ...
        if (amount % 100 == 0) {
            if (acc.balance >= amount) { 
               ...
               try {
                   ...
                   //Systemm.out.println("The File Name - " + fileName);
                   FileWriter fileWriter = new FileWriter(fileName, true);
                   BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
                   //Write the transaction details to the text file
                   bufferedWriter.write("Date: " + new Date() + "\n");
                   bufferedWriter.write("Witdrawal: " + amount + "\n");
                   bufferedWriter.write("Account Number: " + acc.a_no + "\n");
                   bufferedWriter.write("Reamining Balance: " + acc.balance + "\n\n");
                   // Close the file writer and buffered writer
                   bufferedWriter.close();
                   fileWriter.close();
                } catch (IOException e) {
                    System.out.println("An error occurred while writeing to the file.");
                    e.printStackTrace();
                }
                ...
                System.out.println("!!!!!!!! Insuffiicient Funds !!!!!!!!");
            }
        } else {
            System.out.println("Ammount not in multiples of 100 !");
    ...
    void deposit(Account acc) {
        ...
        System.out.println("Processing Your Request! Plese Wait ...");
        try {
            Thread.sleep(5000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        ...
        System.out.println("\033[H\033[2J");
        System.out.flush();
        System.out.println("******Transaction complleted Successfully!******");
    ...
    void Transfer(Account acc1, Account acc2, double amount) {
        if (acc1.balance >= amount) {
            ...
            try {
                ...
                System.out.println("An error occurred while to the file.");
                e.printStackTrace();
            }
            ...
            System.out.println("\033[H\033[2J");
            System.out.flush();
            System.out.println("\nAccount Transfer completed Sucessfully !!!\n--------Going to Login Page--------");
            try {
                Thread.sleep(5000);
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    void dispaly_details(Account acc){
        System.out.println("\033[H\033[2J");
        System.out.flush();
        ...
        System.out.println("Screen will autommatically timeout after 15 seconds ...");
        try {
            Thread.sleep(15000);
        } catch (InterruptedException e) {
            e.printStackTrace();
...
class run_atm {
    int acount_search_by_unique_id(Account[] ac, String unique_id) {
        for (int i = 0; i < 5; i++) {
    ...
    void demo(Account[] ac) {
        ...
        int i = acount_search_by_unique_id(ac, unique_id);
        if (i == -1) {
            System.out.println("Account not register Yet!");
            try {
                Thread.sleep(3000);
            } catch (InterruptedException e) {
                e.printStackTrace();
            ...
            if(pin == ac[i].pin) {
                ...
                switch (ch) {
                    ...
                    case 4 -> atm.dispaly_details(ac[i]);
                    case 5 -> atm.quit();
                }
